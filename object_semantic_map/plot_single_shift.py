...
parser.add_argument("-o", type=int, help="object index")
...
for f in glob.glob(os.path.join(args.f, "*.npy")):
    print(f)
    #if "FloorPlan1" not in file: continue
    np_map = np.load(f)
    fig, ax = plt.subplots(1, 1)
    obj_id = args.o
    loc = np.where(np_map[:,:,obj_id+1]>0)
    new_map = np.copy(np_map)
    print (loc)
    for i in range(np.where(np_map[:,:,obj_id+1]>0)[0].shape[0]):
        x = np.where(np_map[:,:,obj_id+1]>0)[0][i]
        y = np.where(np_map[:,:,obj_id+1]>0)[1][i]
        print(x)
        print(y)
        if y>5: new_map[x,y,obj_id+1] = 0
        #new_map[x,y-3,obj_id+1] = np_map[x,y,obj_id+1]
    pcm = ax.pcolor(new_map[:,:,obj_id+1]*0.2, cmap=plt.get_cmap('Reds'), vmin=-0, vmax=1)
    #print(np_map[np_map[:,:,obj_id]>0])
    ax.set_title(constants.OBJECTS[obj_id], fontsize=20)
    ax.axis('off')
    """
    ...
    #plt.show()
    plt.tight_layout()
    plt.savefig("%s_%s_shift.png" % (f.split('.')[0], constants.OBJECTS[obj_id]), dpi = 1000)
    print("%s.png" % (f.split('.')[0]), "saved")
    plt.close()
    #break
